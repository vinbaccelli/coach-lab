...
function Home() {
  ...
  // OFF by default, and reset to OFF every time the skeleton is (re)enabled —
  // see the effect below that pins this to `skeletonOn`. Turning it on swaps the
  // LIVE pose model to MediaPipe (see handleSkeletonShowFootLineChange), so a
  // stale "on" carried over from a previous state left the skeleton showing a
  // MoveNet body with no feet: the MoveNet worker's own ready-snap (Canvas
  // bridge.onReady) runs unconditionally and typically wins the race against
  // MediaPipe's slower WASM+GPU init, so the toggle displayed on while the live
  // pose was actually still MoveNet. Defaulting off — and PINNING off on every
  // skeleton-enable — means MoveNet is always what's actually running at that
  // moment, so there is nothing for that race to get wrong. The very next
  // explicit off→on IS a real change event and engages MediaPipe cleanly (fires
  // the popup, warms the landmarker) exactly as intended.
  ...
  /**
   * Foot lines swap the LIVE pose model (MoveNet → MediaPipe). That is a real
   * trade the coach has to be told about ONCE: it costs live smoothness, and it
   * changes the joint-angle numbers they may be reading, because a different
   * model is placing the joints. Dismissible for good via localStorage.
   */
  const FOOT_LINE_NOTICE_KEY = 'footLineLiveNoticeDismissed';
  const [footLineNoticeOpen, setFootLineNoticeOpen] = useState(false);
  /** Set when the device cannot sustain live MediaPipe and we reverted to MoveNet. */
  const [footLineUnsupported, setFootLineUnsupported] = useState<string | null>(null);
  ...
  const resolveExactPoseAt = useCallback(async (
    ...
    // `name` is carried through: the MediaPipe foot landmarks past index 16 are
    // identified only by name, and the Motion Layer zone now reads them.
  ...
  const buildPlayerFrameSpec = useCallback(async (index: number): Promise<StroAutoFrameSpec | null> => {
    ...
    return {
      ...
      // `name` is CARRIED, not dropped: the MediaPipe foot landmarks live past
      // index 16 and are identified only by name, so stripping it here made the
      // real toe invisible to the zone, which then had to guess the foot with a
      // symmetric disc at the ankle. Every COCO-17 slot is still positional.
      keypoints: kps.map((k) => ({ x: k.x / vw, y: k.y / vh, score: k.score ?? 0, name: k.name })),
    };
  ...
  const handleStroGenerate = useCallback(async () => {
    ...
    if (!pngUrl) {
      setStroPreviewError('Could not build the Motion Layer image. Close and try Generate again.');
      return;
    }
    setStroPreviewPngUrl(pngUrl);
    setStroPreviewError(null);
  ...
  useEffect(() => {
    ...
    setSessionDraftTitle(`${localDateTimeForFolder()} — Motion Layer`);
  }, [
    applyStroMotionToDraft,
  ...
  const handleStroDownloadPng = useCallback(() => {
    if (stroPreviewPngUrl) {
      downloadDataURL(stroPreviewPngUrl, `motion-layer-${Date.now()}.png`);
      return;
    }
    const video = videoRef.current;
    if (video && stroMotionDraft) {
      void exportStroMotionDraftPng(video, stroMotionDraft).then((url) => {
        downloadDataURL(url, `motion-layer-${Date.now()}.png`);
      }).catch(() => {
        setProcessingStatus('Could not export PNG. Generate Motion Layer first.');
      });
      return;
    }
    setProcessingStatus('Generate Motion Layer first.');
  ...
  const handleStroDownloadVideo = useCallback(() => {
    ...
    a.href = url;
    a.download = `motion-layer-${Date.now()}.webm`;
  ...
  const stroMotionPanelEl = (
    <StroMotionPanel
      ...
      disabledReason={
        !stroMotionHtml5Only
          ? 'Motion Layer requires an uploaded video file (not YouTube or embed links).'
          : undefined
      }
    />
  );
  ...
  // Foot line is PINNED off on every skeleton-enable — see the state comment
  // above `skeletonShowFootLine` for why a stale "on" is unsafe. `skeletonOn`
  // starts false and this effect only re-fires when it actually TRANSITIONS to
  // true, so it fires once per activation (including re-activation after the
  // coach explicitly turns skeleton off) without touching a coach's in-session
  // foot-line choice while skeleton stays on — e.g. it will not undo the coach
  // turning foot lines ON a moment later. Deliberately the raw setter, not
  // `handleSkeletonShowFootLineChange`: this is a silent reset, not a coach
  // decision, so it must not fire the popup or clear the unsupported banner.
  useEffect(() => {
    if (skeletonOn) setSkeletonShowFootLine(false);
  }, [skeletonOn]);
  ...
  /**
   * Turning foot lines ON switches the live pose model, so show the disclosure
   * once (unless dismissed for good). Turning them OFF is free — no notice.
   */
  const handleSkeletonShowFootLineChange = useCallback((on: boolean) => {
    setSkeletonShowFootLine(on);
    if (!on) return;
    setFootLineUnsupported(null); // a fresh opt-in re-arms the capability check
    let dismissed = false;
    try { dismissed = localStorage.getItem(FOOT_LINE_NOTICE_KEY) === '1'; } catch { /* blocked */ }
    if (!dismissed) setFootLineNoticeOpen(true);
  }, []);

  /** Canvas reverted live MediaPipe → MoveNet because this device can't sustain it. */
  const handleFootLineLiveUnsupported = useCallback((reason: string) => {
    setFootLineUnsupported(reason);
  }, []);
...
onTrimChange={analysisTimelineExtras.onTrimChange}
  ...
  const toolPaletteBaseProps = {
    ...
    onSkeletonShowFootLineChange:    handleSkeletonShowFootLineChange,
      ...
      if (screen === 'stromotion') {
        ...
        if (biomechPhaseMarkers && biomechPhaseMarkers.length > 0) {
          ...
          if (first < last) {
            ...
            setProcessingStatus(`Imported ${count} phase markers as Motion Layer frames`);
          }
        }
  ...
  return (
    ...
    >
            ...
            >
              ...
              >
                ...
                <CanvasOverlay
                  ...
                  onFootLineLiveUnsupported={handleFootLineLiveUnsupported}
      ...
      {stroEditingFrameIndex !== null && stroMotionDraft ? (() => {
        ...
        return (
          <FrameMaskEditor
            ...
            // SAM racket embedding key. This is the only place that holds both
            // the frame index and its timeSec, so the key is built here rather
            // than reconstructed inside the editor from partial information.
            racketKey={samRacketFrameKey(frame.index, frame.timeSec)}
      ...
      {footLineNoticeOpen && createPortal(
        <div
          style={{
            position: 'fixed', bottom: 140, left: '50%', transform: 'translateX(-50%)',
            zIndex: 9500, background: '#1D1D1F', borderRadius: 16,
        ...
        >
          <span style={{ fontSize: 14, fontWeight: 600, color: '#fff', textAlign: 'center' }}>
            Foot lines use the full-body model
          </span>
          <span style={{ fontSize: 13, color: '#C7C7CC', textAlign: 'center', lineHeight: 1.45 }}>
            Joint angles may read slightly differently, and live playback may be less
            smooth. For smooth playback with feet, run AI&nbsp;Track.
          </span>
          <div style={{ display: 'flex', gap: 10, width: '100%' }}>
            <button
              type="button"
              onClick={() => setFootLineNoticeOpen(false)}
              style={{
            ...
            <button
              type="button"
              onClick={() => {
                try { localStorage.setItem(FOOT_LINE_NOTICE_KEY, '1'); } catch { /* blocked */ }
                setFootLineNoticeOpen(false);
              }}
              style={{
                flex: 1, padding: '10px 0', borderRadius: 10, border: '1px solid #555',
                background: 'transparent', color: '#C7C7CC', fontSize: 13, fontWeight: 600, cursor: 'pointer',
              }}
            >
              Don&apos;t show again
            </button>
      ...
      {footLineUnsupported && createPortal(
        <div
          style={{
            position: 'fixed', bottom: 140, left: '50%', transform: 'translateX(-50%)',
            zIndex: 9500, background: '#3A2A1D', border: '1px solid #FF9500', borderRadius: 16,
            padding: '16px 24px', boxShadow: '0 8px 32px rgba(0,0,0,0.4)',
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12,
            maxWidth: 380, width: '92%',
          }}
        >
          <span style={{ fontSize: 14, fontWeight: 700, color: '#FFB340', textAlign: 'center' }}>
            This device can&apos;t run foot lines smoothly live
          </span>
          <span style={{ fontSize: 13, color: '#E9DCCB', textAlign: 'center', lineHeight: 1.45 }}>
            The live skeleton has switched back to the fast model. Run AI&nbsp;Track to get
            real feet with smooth playback.
          </span>
          <button
            type="button"
            onClick={() => setFootLineUnsupported(null)}
            style={{
              width: '100%', padding: '10px 0', borderRadius: 10, border: 'none',
              background: '#FF9500', color: '#1D1D1F', fontSize: 14, fontWeight: 700, cursor: 'pointer',
            }}
          >
            OK
          </button>

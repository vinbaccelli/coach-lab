...
/**
 * FREE THE DETECTOR — release its ONNX session, GPU buffers and wasm arena.
 *
 * THIS IS NOT HOUSEKEEPING; IT IS THE POINT OF THE PHASED BATCH. D-FINE and
 * SAM-2 are both multi-hundred-MB ONNX sessions, and on a wasm / low-VRAM
 * machine having both resident while a THIRD model (the MediaPipe selfie
 * segmenter) is doing the person cutout is the memory-pressure condition we are
 * removing. The batch runs all detections, calls this, and only then loads SAM,
 * so at no instant do two heavy models coexist.
 *
 * `PreTrainedModel.dispose()` walks `this.sessions` and calls `release()` on
 * each — the real ORT `InferenceSession.release(): Promise<void>`, not a no-op —
 * so this genuinely returns memory rather than merely dropping a reference.
 *
 * Nulls `sessionPromise` too, so a later batch reloads cleanly instead of
 * handing out a disposed session. Idempotent and never throws: a failure to free
 * must not break a batch that has already produced its detections.
 */
export async function disposeRacketDetector(): Promise<void> {
  ...
  try {
    ...
    if (session?.model?.dispose) {
      ...
      console.log('[autoRacket] D-FINE disposed — ONNX session released before SAM loads');
    }
  } catch (e) {
    console.warn('[autoRacket] detector dispose failed (continuing):', e);
  }
}

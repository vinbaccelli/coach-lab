...
export default function AuthButton({ iconOnly = false }: AuthButtonProps) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        <span style={{ ...iconWrap, overflow: 'hidden', borderRadius: '50%', background: 'var(--cl-accent)' }}>
          {avatarUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={avatarUrl} alt={initials} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--cl-text-on-fill)' }}>{initials}</span>
          )}
        </span>
        {iconOnly ? null : (
      ...
      {showMenu ? (
        ...
        >
          <div style={{ padding: '6px 10px 10px', borderBottom: '1px solid var(--cl-bg-secondary)', marginBottom: 6 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--cl-text-primary)' }}>
              {(user.user_metadata?.full_name as string | undefined) ?? 'Signed in'}
            </div>
            <div style={{ fontSize: 11, color: 'var(--cl-text-secondary)', marginTop: 2 }}>{user.email}</div>
          </div>
          <button
            type="button"
            style={{ ...menuItem, color: 'var(--cl-destructive)' }}
            disabled={signingOut}
            onClick={async () => {
              setSigningOut(true);
              setShowMenu(false);
...
function btnBase(active: boolean): React.CSSProperties {
  return {
    ...
    color: 'var(--cl-text-primary)', textAlign: 'left',
  };
}
...
const menuItem: React.CSSProperties = {
  ...
  fontSize: 13, fontWeight: 500, color: 'var(--cl-text-primary)',
};
